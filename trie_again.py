class Trie:
    ...
    def dfs(self,d):
        if len(d) == 1 and '*' in d:
            return d['*']
        if '*' in d :
            return d['*'] + sum([self.dfs(d[n]) for n in d if n != '*'])

        return sum([self.dfs(d[n]) for n in d])
